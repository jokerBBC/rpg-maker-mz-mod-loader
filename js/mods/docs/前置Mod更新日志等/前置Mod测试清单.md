...
| # | 测试项 | 测试方式 | 预期结果 | 状态 |
|---|--------|----------|----------|------|
| R20 | 双 Mod 同路径冲突 | 两启用 Mod 的 resources 指向同一 originalPath | getConflictReport 有条目；高 order 生效 | 未测 |
| R21 | 无冲突时面板 | 仅一个 Mod 替换或 URL 相同 | 面板显示「无冲突」 | 未测 |
| R22 | order 决定胜者 | 低 order 与高 order 替换同资源 | 面板胜者为高 order Mod | 未测 |
...
| # | 测试项 | 测试方式 | 预期结果 | 状态 |
|---|--------|----------|----------|------|
| L1 | 冲突日志面板注册 | 启用 ModDataLoader → 打开管理器 → 点设置齿轮 | 菜单底部出现「数据冲突日志」；未启用时无此项 | ✅ 已测 |
| L2 | 冲突面板与提醒 | 有冲突时看齿轮红叹号；点菜单项打开面板 | 红叹号出现；面板由 DataLoader 渲染报告 | ✅ 已测 |
| L7 | 资源冲突日志入口 | 启用 ModResourceLoader → 打开管理器 → 点设置齿轮 | 菜单底部出现「资源冲突日志」；未启用时无此项 | 未测 |
| L8 | 资源冲突面板 | 两 Mod 替换同一资源 → 开面板 | 显示类型/路径/胜者/已被覆盖；有冲突时齿轮红叹号 | 未测 |
